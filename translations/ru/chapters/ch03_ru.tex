...
Формально рассмотрим последовательность случайных величин $S_0, A_0, R_1, S_1, A_1, \dots$, представляющих состояние, действие и вознаграждение на каждом шаге. Полная история до момента $t$ — кортеж
\[
...
Для многих естественных представлений состояния марковское свойство является хорошим приближением, а не точной истиной. Погодные паттерны имеют дальноиграющие зависимости; положение робота зависит от износа суставов. На практике мы выбираем представление состояния максимально информативным при условии вычислительной приемлемости.
...
\begin{description}[
  ...
  \item[$p(s',r \mid s, a)$] — \textbf{ядро перехода-вознаграждения}\index{transition kernel} (модель среды): вероятность того, что выполнение действия $a$ в состоянии $s$ приводит к следующему состоянию $s'$ и вознаграждению $r \in \cR \subset \R$;
...
\begin{itemize}[leftmargin=*]
  ...
  \item $s_2$: \textbf{Рутуб} — просмотр социальных сетей.
...
$s_1$ (Занятия) & отвлечься & $s_2$ (Рутуб) & 1.0 & $-1$ \\
$s_2$ (Рутуб) & выйти & $s_1$ (Занятия) & 1.0 & $-1$ \\
$s_2$ (Рутуб) & продолжить & $s_2$ (Рутуб) & 1.0 & $-1$ \\
...
\begin{tikzpicture}[
    >=Stealth,
    state/.style={draw, rounded corners, minimum width=2.0cm, minimum height=0.9cm,
                  align=center, font=\small, fill=blue!5},
    terminal/.style={state, double, double distance=1.2pt, fill=blue!10},
    every path/.style={->, thick, >=Stealth},
    lab/.style={font=\small, inner sep=1.5pt, fill=white}
  ]
  % Узлы — компактнее по горизонтали, сдвиг влево (центр смещён к x=-0.5)
  \node[state]    (fb) at (-3.2,  0.0) {Рутуб $s_2$};
  \node[state]    (c)  at ( 0.0,  1.8) {Занятия $s_1$};
  \node[state]    (st) at ( 3.2,  0.0) {Учёба $s_3$};
  \node[terminal] (sl) at ( 3.2, -2.8) {Сон $s_4$};

  \path
    (c)  edge[bend left=18]  node[lab, sloped, above] {посещать, $-2$}   (st)
    (c)  edge[bend right=18] node[lab, sloped, above] {отвлечься, $-1$}  (fb)
    (fb) edge[bend right=18] node[lab, sloped, below] {выйти, $-1$}      (c)
    (fb) edge[loop left]     node[lab, left=2pt]      {продолжить, $-1$} ()
    (st) edge                node[lab, right=2pt]     {сдать, $+10$}     (sl)
    (st) edge[loop right]    node[lab, right=2pt]     {учиться, $-2$} ();
\end{tikzpicture}
...
\end{align*}
Для $V_*(s_2)$ заметим, что из Рутуб оптимальное действие — \emph{выйти} (возвращаясь к Занятиям, имеющим ценность $7.0$), поэтому:
\[
