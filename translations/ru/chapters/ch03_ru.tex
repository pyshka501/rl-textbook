...
Рассмотрим четырёхклеточную решётку с клетками $\{(0,0),(0,1),(1,0),(1,1)\}$ и действиями $\{\text{Вверх, Вниз, Влево, Вправо}\}$. Детерминированная политика может отображать $(0,0) \mapsto \text{Вправо}$, $(0,1) \mapsto \text{Вверх}$, $(1,0) \mapsto \text{Вправо}$, $(1,1)\mapsto\text{Вверх}$. Стохастическая политика может назначать равную вероятность $0.25$ каждому действию в каждом состоянии\,---\,равномерное случайное блуждание.
\end{example}
...
\begin{align*}
V_*(s_4) &= 0 \quad \text{(поглощающее)}, \\
V_*(s_3) &= r(\text{сдать}) + \gamma V_*(s_4) = 10 + 0.9 \times 0 = 10.0, \\
V_*(s_1) &= r(\text{посещать}) + \gamma V_*(s_3) = -2 + 0.9 \times 10 = -2 + 9 = 7.0.
\end{align*}
...
В $s_1$:
\begin{align*}
Q_*(s_1, \text{посещать})   &= -2 + 0.9 \times 10 = 7.0, \\
Q_*(s_1, \text{отвлечься}) &= -1 + 0.9 \times V_*(s_2).
\end{align*}
...
Следовательно:
\[
Q_*(s_1, \text{отвлечься}) = -1 + 0.9 \times 5.3 = -1 + 4.77 = 3.77.
\]
Поскольку $Q_*(s_1, \text{посещать}) = 7.0 > 3.77 = Q_*(s_1, \text{отвлечься})$, жадная политика действительно выбирает «посещать», подтверждая наши прежние рассуждения.

\paragraph{Проверка уравнения оптимальности Беллмана в $s_2$.}
Уравнение оптимальности Беллмана~\eqref{eq:bellman-opt-v} гласит:
\[
V_*(s_2)
= \max\bigl\{Q_*(s_2, \text{выйти}),\, Q_*(s_2, \text{продолжить})\bigr\}.
\]
Имеем:
\begin{align*}
Q_*(s_2, \text{выйти}) &= -1 + 0.9 \times 7.0 = 5.3, \\
Q_*(s_2, \text{продолжить}) &= -1 + 0.9 \times V_*(s_2) = -1 + 0.9 \times 5.3 = 3.77.
\end{align*}
