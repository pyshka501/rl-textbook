...
\begin{tikzpicture}[
  ...
  \node[label, above=0.25cm of A] {Function\\Approximation};
  \node[label, below left=0.2cm of B] {Bootstrapping};
  \node[label, below right=0.2cm of C] {Off-Policy\\Training};

  \node[font=\itshape\small, align=center] at ($(A)!0.5!(B)!0.33!(C)$)
    {Potential\\Divergence};

  % Safe-zone annotations on edges
  \node[font=\tiny, above right, rotate=60] at ($(A)!0.5!(B)$) {MC (no bootstrap)};
  \node[font=\tiny, above, rotate=0] at ($(B)!0.5!(C)$) {Tabular (no approx.)};
  \node[font=\tiny, above left, rotate=-60] at ($(A)!0.5!(C)$) {On-policy TD};
\end{tikzpicture}
